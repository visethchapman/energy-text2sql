"""Base Qwen2.5-Coder-1.5B (no LoRA adapter) — the un-finetuned reference.

Used to isolate what the ERCOT LoRA adapter actually adds on top of the
raw pre-trained base. Same Agent protocol, same system prompt, same
generation config as agent/finetuned.py — only the PEFT adapter is
skipped so we're benchmarking the base weights directly.
"""
...
def _load(base_model: str = BASE_MODEL):
    ...
    _MODEL = AutoModelForCausalLM.from_pretrained(base_model, torch_dtype=dtype).to(device)
    _MODEL.eval()
    return _MODEL, _TOKENIZER
